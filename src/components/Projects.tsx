...
function ProjectCard({ project }: { project: Project }) {
  ...
  // An image entry is treated as a real image if it starts with "/" (public folder path)
  // or "http" (remote URL). Otherwise it's treated as an emoji/text placeholder.
  const isRealImage =
    ...
    >
      <div>
        ...
        <div className="h-44 bg-bg-primary border-b border-white/5 relative flex items-center justify-center text-5xl overflow-hidden">
          ...
          <div className="absolute top-3 right-3 font-ui text-[9px] uppercase tracking-widest text-accent-blue px-2 py-0.5 rounded bg-accent-blue/10 border border-accent-blue/20 z-20">
            {project.category}
          </div>
        </div>
